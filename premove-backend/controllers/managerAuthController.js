...
export const getDashboardCustomersBySpanco = async (req, res) => {
  try {
    ...
    if (!spanco)
      return res
        .status(400)
        .json({ success: false, error: "Missing spanco value" });
...
export const getManagerCustomers = async (req, res) => {
  try {
    const managerId = req.user.id; // 👈 middleware se aapke manager ka id aa raha hoga
    ...
    let where = "";
    let params = [assignLocation]; // 👈 first param hamesha location match karega

    if (search) {
      where = `AND (r.full_name LIKE ? OR r.customer_email LIKE ? OR r.customer_mobile_no LIKE ?)`;
      params.push(search, search, search);
    }

    // Customers with only matched leads
    const [customers] = await db.promise().query(
      `
      SELECT r.id, r.full_name, r.customer_email, r.customer_mobile_no, r.profile_photo,
             COUNT(l.id) as total_leads
      FROM ele_customer_register r
      LEFT JOIN ele_customer_lead l 
        ON l.cust_mobile = r.customer_mobile_no 
       AND l.city_name = ?   -- 👈 only leads matching manager location
      WHERE 1=1
      ${where}
      GROUP BY r.id
      LIMIT ? OFFSET ?
    ...
    // Total customers count (only those who have leads in assign_location)
...
// ✅ Fetch all leads for a registered customer
export const getCustomerLeads = async (req, res) => {
  try {
    ...
    const [leads] = await db
      .promise()
      .query(
        `SELECT * FROM ele_customer_lead WHERE cust_mobile = ? ORDER BY lead_date DESC`,
        [mobile]
      );
